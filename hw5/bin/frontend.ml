...
let oat_alloc_struct (ct:TypeCtxt.t) (id:Ast.id) : Ll.ty * operand * stream =
  let src_struct_ty = TypeCtxt.lookup id ct in
  let src_struct_size = Const(Int64.of_int(8 * List.length(src_struct_ty))) in
  let src_struct = gensym "src_struct" in
  let alloc_struct = gensym "alloc_struct" in
  let alloc_struct_type = cmp_ty ct (TRef (RStruct id)) in
  alloc_struct_type, Id alloc_struct, lift[(src_struct, Call(Ptr I64, Gid "oat_malloc", [I64, src_struct_size]));
  (alloc_struct, Bitcast(Ptr I64, Id src_struct, alloc_struct_type))
  ]
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    ...
    let ans_ty = match exp_ty with
    | Ptr (Struct [_; Array(_, ty)]) -> ty
    | _ -> failwith "not a pointer: fail check 1"
  in
    let start_ptr, ans_id = gensym "ptr", gensym "ans_id" in
    ans_ty, Id ans_id, exp_stream >:: I(start_ptr, Gep(exp_ty, exp_op, [Const 0L; Const 0L])) >:: I(ans_id, Load(Ptr I64, Id start_ptr))
  ...
  *)
  | Ast.NewArrInit (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let arr_ptr = gensym "e1_arr_ptr" in 
    let arr_length = gensym "arr_length" in
    let arr_return = gensym "arr_return" in
    let arr_ptr_stream = lift[arr_ptr, Alloca arr_ty;("", (Store (arr_ty, arr_op, Id arr_ptr)))] in
    let arr_length_stream = lift[arr_length, Alloca I64;("", (Store (I64, size_op, Id arr_length)))] in
    
    let ctxt1 = Ctxt.add c arr_ptr ((Ptr arr_ty), Id arr_ptr) in
    let ctxt2 = Ctxt.add ctxt1 arr_length ((Ptr I64), Id arr_length) in
    
    let init = [id, no_loc(CInt 0L)] in
    let condition = Some (no_loc (Bop (Lt, no_loc (Id id), no_loc (Id arr_length)))) in
    let step_length = no_loc (Bop (Add, no_loc(Id id), no_loc (CInt 1L))) in
    let update_step = Some (no_loc @@ Assn ((no_loc @@ Id id),step_length)) in
    let exp_body = [no_loc (Assn (no_loc (Index (no_loc (Id arr_ptr), no_loc (Id id))), e2))] in
    let for_loop = no_loc (For (init, condition, update_step, exp_body)) in
    let (_, for_stream) = cmp_stmt tc ctxt2 arr_ty for_loop in

    let return_arr_stream = [I(arr_return, (Load (Ptr arr_ty, Id arr_ptr)))] in

    arr_ty, Id arr_return, size_code >@ alloc_code >@ arr_ptr_stream >@ arr_length_stream >@ for_stream >@ return_arr_stream 
   ...
   *)
   | Ast.CStruct (id, fields) ->
    let field_types = 
      List.map (fun (name, _) -> TypeCtxt.lookup_field_name id name tc |> fst) fields in
    let str_ty, str_op, alloc_code = oat_alloc_struct tc id in

    let field_code = List.mapi (fun i (name, exp) -> 
          let field_ty = cmp_ty tc (List.nth field_types i) in
          let elt_op, elt_code = cmp_exp_as tc c exp field_ty in
          let ind = gensym "ind" in
          elt_code >@ lift[ind, Gep (str_ty, str_op, [Const 0L; Const (Int64.of_int i)]);
            "", Store (field_ty, elt_op, Id ind)]
          ) fields |> List.flatten
    in
    str_ty, str_op, alloc_code >@ field_code
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
    ...
    | Ptr(Namedt id) -> id
    | _ -> failwith "Not pointer to struct: fail check 3"
  in
    let ans_ty, ans_index = TypeCtxt.lookup_field_name struct_id i tc in
    let ans_id = gensym "ans_id" in
    ((cmp_ty tc ans_ty), Id ans_id, struct_code >:: I(ans_id, Gep(struct_ty, struct_op, [Const 0L; Const ans_index])))
  ...
  *)
  | Ast.Index (e, i) ->
    ...
    let ans_ty = match arr_ty with 
      ...
      | _ -> failwith "Index: indexed into non pointer: fail check 2" in
    let ptr_id, tmp_id, call_id = gensym "index_ptr", gensym "tmp", gensym "call" in
    ans_ty, (Id ptr_id),
    arr_code >@ ind_code
    >@ lift[tmp_id, Bitcast(arr_ty, arr_op, Ptr I64)]
    >@ lift[call_id, Call(Void, Gid "oat_assert_array_length", [Ptr I64, Id tmp_id; I64, ind_op;])]
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    let exp_ty, exp_op, exp_stream = cmp_exp tc c exp in
    let id_alloc = gensym "id_alloc" in
    let ctxt = Ctxt.add c id (Ptr exp_ty, Id id_alloc ) in
    let notnull_block, null_block, merge_block, branch_condition = gensym "notnull", gensym "null", gensym "end", gensym "condition" in
    let condition_stream = lift[branch_condition, Icmp (Ne, exp_ty, exp_op, Null)] >@ [T(Cbr (Id branch_condition, notnull_block, null_block))] in
    let notnull_stream = [L notnull_block] >:: I(id_alloc, Alloca exp_ty) >:: I("", Store (exp_ty, exp_op, Id id_alloc)) >@ snd @@ cmp_block tc ctxt rt notnull >@ [T (Br merge_block)] in
    let null_stream = [L null_block]  >@ snd @@ cmp_block tc c rt null >@ [T (Br merge_block)]  in
    c, exp_stream >@ condition_stream >@ notnull_stream >@ null_stream >@ [L merge_block]
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    ...
    let expdir, gvar = List.fold_left(fun (expdir, gvar) field -> 
      match List.find_opt (fun (cs_id, _) -> cs_id = field.fieldName) cs with
      | Some (_, cs_exp) ->
        let index = TypeCtxt.index_of_field id field.fieldName tc in
        let gval, gvar' = cmp_gexp c tc cs_exp in
        ((index, gval) :: expdir, gvar' @ gvar )
      | None -> (expdir, gvar)
    ) ([], []) fields
    in
    let expList = expdir |> List.sort(fun (index1, _) (index2, _) -> compare index1 index2)
                         |> List.map snd
    in

    let gid = gensym "gid" in
    (Ptr (Namedt id), GGid gid), (gid, (Namedt id, GStruct expList))::gvar
