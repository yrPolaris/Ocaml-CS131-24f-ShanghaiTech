...
type itf_node = UidSet.t * Alloc.loc option * int
type itf_graph = itf_node UidMap.t

let insn_assigns : Ll.insn -> bool = function
  | Ll.Call (Ll.Void, _, _) | Ll.Store _ -> false
  | _ -> true

let init_itf_graph (f:Ll.fdecl) (live:liveness) : itf_graph =
  let init_labels (g:itf_graph) =
    let labels = UidSet.of_list (List.map fst (snd f.f_cfg)) in
    UidSet.fold (fun lbl g -> 
      let loc = Some (Alloc.LLbl (Platform.mangle lbl)) in
      UidMap.add lbl (UidSet.empty, loc, 0) g) labels g
  in
  let init_params (g:itf_graph) : itf_graph =
    let params = UidSet.of_list f.f_param in
    let regs = [Rdi; Rsi; Rdx; Rcx; R08; R09] in
    let regsl = List.init (List.length f.f_param) (fun i -> if i < 6 then List.nth regs i else Rcx)
  in
    List.fold_left2 (fun g_in param reg ->
      let itf = UidSet.remove param params in
      let loc = if reg = Rcx then None else Some (Alloc.LReg reg) in
      UidMap.add param (itf, loc, 5) g_in
    ) g f.f_param regsl
  in
  let init_insns (g:itf_graph) : itf_graph =
    let ex_insn_uidset (i: Ll.insn) : UidSet.t =
      let collect_uids = function
        | Id uid -> Some uid
        | _ -> None
      in
      let ops = match i with
        | Binop (_, _, op1, op2)
        | Icmp (_, _, op1, op2)
        | Store (_, op1, op2) -> [op1; op2]
        | Load (_, op)
        | Bitcast (_, op, _) -> [op]
        | Call (_, op, ops) -> op :: List.map snd ops
        | Gep (_, op, ops) -> op :: ops
        | _ -> []
      in
      ops |> List.filter_map collect_uids |> UidSet.of_list
    in
    let add_dep (dep:uid) (curr:uid) ((m, s):(itf_graph * UidSet.t)) : (itf_graph * UidSet.t) =
    ...
    in
    let solve_insn ((uid, insn):(uid * Ll.insn)) (g:itf_graph) : itf_graph =
      let g = UidSet.fold (fun u g -> begin
        match UidMap.find_opt u g with
          | Some(set, reg, cnt) -> UidMap.add u (set, reg, cnt + 1) g
          | None -> UidMap.add u (UidSet.empty, None, 1) g
      end) (ex_insn_uidset insn) g in
      if insn_assigns insn
        then
          let out_set = UidSet.remove uid (live.live_out uid) in
          let (g', init_set) = UidSet.fold (fun dep gs -> add_dep dep uid gs) out_set (g, UidSet.empty) in
          match UidMap.find_opt uid g' with
            | Some((old_set, reg, cnt)) -> UidMap.add uid ((UidSet.union old_set init_set), reg, cnt + 1) g'
            | None -> UidMap.add uid (init_set, None, 0) g'
        else 
          UidMap.add uid (UidSet.empty, Some Alloc.LVoid, 0) g
    in
    let handle_call_params (g:itf_graph) (insn:Ll.insn) =
      match insn with
      | Ll.Call (_, _, ops) ->
        let regs = [Rdi; Rsi; Rdx; Rcx; R08; R09] in
        let regsl = List.init (List.length ops) (fun i -> if i < 6 then List.nth [Rdi; Rsi; Rdx; Rcx; R08; R09] i else Rcx) in
        List.fold_left2 (fun g (_, op) reg ->
          match op with
          | Id uid -> 
            let (set, loc, cnt) = UidMap.find uid g in
            let neighbor_colors = UidSet.fold (fun n acc ->
              match UidMap.find_opt n g with
                | Some (_, Some reg, _) -> LocSet.add reg acc
                | _ -> acc
            ) set LocSet.empty in
            if LocSet.mem (Alloc.LReg reg) neighbor_colors then g
              else
              begin match loc with
              | Some (Alloc.LReg _) -> g
              | _ -> UidMap.add uid (set, Some (Alloc.LReg reg), cnt) g
            end
          | _ -> g
        ) g ops regsl
      | _ -> g
    in
    let init_block (block:Ll.block) (g:itf_graph) : itf_graph =
      List.fold_left (fun g i -> solve_insn i (handle_call_params g (snd i))) g block.insns
    in
    List.fold_left (fun g b -> init_block b g) g ((fst f.f_cfg)::(List.map snd (snd f.f_cfg)))
  in
  let init_term (g:itf_graph) : itf_graph =
    let ex_term_uidset (t:Ll.terminator) : UidSet.t * uid option=
      match t with
        | Ret (_, Some (Id uid) ) -> UidSet.of_list [uid], Some uid
        | Ret (_, Some (Gid uid) ) -> UidSet.of_list [uid], Some uid
        | Cbr (Id uid, _, _) -> UidSet.of_list [uid], None
        | _ -> UidSet.empty, None
    in
    let (term_uidset,ret_uid) = ex_term_uidset (snd (fst f.f_cfg).term) in
    if Option.is_some ret_uid
      then UidSet.fold (fun tu g -> 
              match UidMap.find_opt tu g with
                | Some (set, reg, cnt) -> 
                  if tu = Option.get ret_uid
                    then UidMap.add tu (set, Some (Alloc.LReg Rax), cnt + 1) g
                    else UidMap.add tu (set, reg, cnt + 1) g
                | None -> 
                  if tu = Option.get ret_uid
                    then UidMap.add tu (UidSet.empty, Some (Alloc.LReg Rax), 1) g
                    else UidMap.add tu (UidSet.empty, None, 1) g
            ) term_uidset g
      else UidSet.fold (fun tu g -> 
          match UidMap.find_opt tu g with
            | Some (set, reg, cnt) -> 
              UidMap.add tu (set, reg, cnt + 1) g
            | None -> UidMap.add tu (UidSet.empty, None, 1) g
        ) term_uidset g
  in
  UidMap.empty
  |> init_labels
...
let solve_itf_graph (graph:itf_graph) (available:LocSet.t) : (itf_graph * UidSet.t) =
  let get_available_color node graph available =
    let (neighbors,_,_) = UidMap.find node graph in
    let used_colors = UidSet.fold (fun n acc ->
      match UidMap.find_opt n graph with
        | Some (_,Some reg,_) -> LocSet.add reg acc 
        | _ -> acc
    ) neighbors LocSet.empty in
    let free_colors = LocSet.diff available used_colors in
    if LocSet.is_empty free_colors 
      then None
      else 
        let sorted_colors = 
          List.sort (fun a b ->
            let order = [Alloc.LReg Rdi; Alloc.LReg Rsi; Alloc.LReg Rdx; Alloc.LReg Rcx; Alloc.LReg R08; Alloc.LReg R09; Alloc.LReg R10; Alloc.LReg R11] in
            let index_of x = 
              ...
              aux 0 order
            in
            compare (index_of a) (index_of b)
          ) (LocSet.elements free_colors) in
        Some (List.hd sorted_colors)
  in
  let color_graph_main available graph =
    let sorted = 
      UidMap.bindings graph
      |> List.sort (fun (_, (set1, _, cnt1)) (_, (set2, _, cnt2)) ->
        let degree1 = UidSet.cardinal set1 in
        let degree2 = UidSet.cardinal set2 in
        if degree1 = degree2 then compare cnt1 cnt2 else compare degree1 degree2)
    in
    List.fold_left (fun (cgraph,spill) (node,(_,old_reg,cnt)) ->
      match old_reg with
        | Some _ -> (cgraph,spill) 
        | None ->
            let color = get_available_color node cgraph available in
            match color with
              | Some color -> 
                let (set,_,cnt) = UidMap.find node cgraph in
                (UidMap.add node (set,Some color,cnt) cgraph, spill)
              | None -> (cgraph, UidSet.add node spill)
    ) 
    (graph,UidSet.empty) sorted
  in
  graph |> color_graph_main available
...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  in
  let graph = try init_itf_graph f live with Not_found -> failwith "not found in init" in
  let (colored_graph, spill) = solve_itf_graph graph pal in
  let ex_loc_graph (itfg: itf_graph) : Alloc.loc UidMap.t =
    UidMap.fold (fun uid (_, reg, _) res -> 
      match reg with 
        | Some r -> UidMap.add uid r res
        | None -> res
      ) itfg UidMap.empty
  in
  let (loc_graph, spill) = UidSet.fold (fun uid (set, offset) -> 
    let next_offset = offset + 1 in
    (UidMap.add uid (Alloc.LStk(-next_offset)) set, next_offset)) spill (ex_loc_graph colored_graph, 0)
  in
  let locs (u:uid) : Alloc.loc =
    match UidMap.find_opt u loc_graph with
      | Some loc -> loc
      | None -> failwith ("uid "^u^" not found in loc_graph")
  in
  { uid_loc=locs; spill_bytes=8*spill }
