      ...
      let worklist = Graph.nodes g in
        let rec process (g : Graph.t) (worklist : Graph.NodeS.t) : Graph.t =
          if Graph.NodeS.is_empty worklist then g
            ...
            let pred = Graph.preds g n in
            let pred_list = List.map (Graph.out g) (Graph.NodeS.elements pred) in
            let in_result = Fact.combine pred_list in
            let out_result = Graph.flow g n in_result in
            let g' = Graph.add_fact n out_result g in
            if Fact.compare old_out out_result = 0 then
              process g worklist'
            else
              let succs = Graph.succs g n in
              let worklist_ = Graph.NodeS.union worklist' succs in
              process g' worklist_
            in 
            process g worklist

      (* let worklist = ref (Graph.nodes g) in
      let rec process (g : Graph.t) : Graph.t =
        if Graph.NodeS.is_empty !worklist then g
        else
          ...
          if Fact.compare old_out out_result = 0 then
            ...
            worklist := Graph.NodeS.union *)
      (* it have something wrong with code above. Maybe types match *)
  end
